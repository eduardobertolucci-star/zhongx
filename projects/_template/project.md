# Project State — [PROJECT NAME]

---

## PROJECT INFO

**PROJECT NAME:** [Full title or working title]
**PROJECT SLUG:** [slug-do-projeto]
**PLATFORM:** [YouTube / TikTok / Other]
**TARGET DURATION:** [X] min
**CREATED:** [Date]
**LAST UPDATE:** [Date]

---

## CURRENT STATE

**CURRENT STAGE:** [BRIEF / CONCEPT PITCH / CEO GATE #1 / SCRIPT / NARRATION / ILLUSTRATION / DIRECTION / REVIEW / CEO FINAL GATE / DELIVERY ASSEMBLY / DELIVERY INTEGRITY CHECK / READY FOR CEO EDITING / COMPLETE]

**CURRENT RESPONSIBLE AGENT:** [CEO / Writer / Narrator / Illustrator / Director / Reviewer]

**STATUS:** [ACTIVE / WAITING FOR CEO / CORRECTIONS IN PROGRESS / HALTED — GRAVE ISSUE / COMPLETE]

**STUDIO SCORE:** [X.X / 10 — or N/A if not yet reviewed]

---

## CEO GATES

| Gate | Status | Date | Notes |
|------|--------|------|-------|
| Gate #1 — Concept Approval | PENDING / APPROVED / REVISION REQUESTED | — | — |
| Extraordinary Gate (if triggered) | N/A / PENDING / RESOLVED | — | — |
| Final Gate — Release Authorization | PENDING / APPROVED | — | — |

---

## STAGE TRACKER

| Stage | Status | Responsible | Completed | Notes |
|-------|--------|-------------|-----------|-------|
| Brief | PENDING / COMPLETE | CEO | — | — |
| Concept Pitch | PENDING / IN PROGRESS / COMPLETE | Writer | — | — |
| CEO Gate #1 | PENDING / APPROVED | CEO | — | — |
| Script | PENDING / IN PROGRESS / APPROVED | Writer | — | — |
| Narration + Audio | PENDING / IN PROGRESS / APPROVED | Narrator | — | — |
| Storyboard + Assets | PENDING / IN PROGRESS / APPROVED / PENDING_INTEGRATION | Illustrator | — | — |
| Editing Blueprint | PENDING / IN PROGRESS / APPROVED | Director | — | — |
| Review | PENDING / IN PROGRESS / COMPLETE | Reviewer | — | — |
| CEO Final Gate | PENDING / APPROVED | CEO | — | — |
| Delivery Assembly | PENDING / IN PROGRESS / COMPLETE | System | — | — |
| Delivery Integrity Check | PENDING / PASS / INCOMPLETE | System | — | — |

---

## APPROVED DECISIONS

*(Record all strategic decisions approved by the CEO — angle choice, tone adjustments, specific creative direction. These cannot be changed without triggering an Extraordinary Gate.)*

1. [Decision description — Date approved]
2.
3.

---

## OPEN ISSUES

*(Active problems awaiting resolution — from any source: Reviewer, CEO, agent flag)*

| # | Issue | Source | Severity | Assigned to | Status |
|---|-------|--------|----------|-------------|--------|
| 1 | [Description] | [Reviewer / Agent flag] | [LEVE / MOD / GRAVE] | [Agent] | OPEN / IN PROGRESS / RESOLVED |

---

## CORRECTION HISTORY

*(Log of completed correction cycles)*

| Cycle | Problem | Agent | Archived version | Resolved date |
|-------|---------|-------|-----------------|--------------|
| 1 | [Brief description] | Writer | script-v1.md | [Date] |

---

## DELIVERY STATUS

**DELIVERY INTEGRITY CHECK:** PENDING / PASS / INCOMPLETE
**DELIVERY STATUS:** NOT ASSEMBLED / ASSEMBLING / COMPLETE / INCOMPLETE
**READY FOR CEO EDITING:** NO / YES

If INCOMPLETE — missing or broken items:
- [ ] [Item / file / reference]

---

## NEXT ACTION

**Who:** [Agent / CEO / System]
**Action:** [Specific next step]
**Waiting for:** [If blocked — what is needed to proceed]
